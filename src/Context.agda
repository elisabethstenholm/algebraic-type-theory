    ...
    where
      ...
      preservesComposition~ f g (inr y) = ap (λ h → inr (h y)) (preserves-composition f g)
        where
          ...
          open Semicategory.Reasoning (TypeSemicategory j)
...
record ContextEquivalence
  ...
  field
    ...
    natural : {j₀ j₁ : Ob 𝒥} (f : Hom 𝒥 j₀ j₁)
            → Δ ⟨ f ⟩ ∘ there (component j₀) ＝ there (component j₁) ∘ Γ ⟨ f ⟩

instance
  sameyContext : ∀ {o a} {𝒥 : Semicategory o a} → Samey Level (Context 𝒥)
